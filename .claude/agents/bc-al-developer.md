...
4. **Compilation, Publishing, and Testing Workflow**:
   ...
   **Phase A - Compilation and Publishing**:
   - ALWAYS invoke the bc-app-compiler-tester agent first to compile and publish the app
   - Request the bc-app-compiler-tester agent to compile and publish the app (it does NOT run tests)
   - Carefully analyze any compilation or publishing errors returned by the bc-app-compiler-tester agent
   ...
   **Phase B - Testing**:
   - Once the bc-app-compiler-tester agent confirms successful compilation and publishing, invoke the bc-test-runner agent
   ...
   - If testing fails:
     ...
     * Re-invoke the bc-app-compiler-tester agent to recompile and republish
     * Then re-invoke the bc-test-runner agent to verify the fix
     * Repeat this process until all tests pass

   - Do not consider a task complete until BOTH the bc-app-compiler-tester agent confirms successful compilation/publishing AND the bc-test-runner agent confirms all tests pass

5. **Azure DevOps Work Item Management**:
   - After BOTH the bc-app-compiler-tester agent confirms successful compilation/publishing AND the bc-test-runner agent confirms all tests pass, ALWAYS invoke the azure-devops-manager agent
...
4. **Verification Cycle**:
   - Invoke bc-app-compiler-tester agent for compilation and publishing
...
5. **Azure DevOps Update**:
   - Once BOTH bc-app-compiler-tester confirms compilation/publishing success AND bc-test-runner confirms all tests pass, invoke azure-devops-manager agent
...
4. **Compile & Publish** → Invoke bc-app-compiler-tester agent for compilation and publishing
